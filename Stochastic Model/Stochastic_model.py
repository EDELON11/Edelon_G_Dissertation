...
#Different combinations of (na) and (nc) per room class
room_combinations = {
    'CR': [(1, 0), (1, 1), (2, 0)],
    'JCR': [(1, 0), (1, 1), (2, 0)],
    'CCR': [(1, 0), (1, 1), (2, 0)],
...
# Total amount of rooms per room-class
...
#Time-instance per day 
t_instances = 4

#Dates of reservation interval
start_reservation_date_str = "2023-03-25"
end_reservation_date_str = "2023-04-07"


#Dates of service interval
start_service_date_str = "2023-04-08"
end_service_date_str = "2023-04-12"
...
def reservation_date_to_time_instances(start_reservation_date_str, end_reservation_date_str):
  
    #Convert a reservation date range to the corresponding time instances. Returns a list of time periods.
    ...
    for day in range(start_day, end_day + 1):  # +1 to include the end day
        for period in range(1, t_instances + 1):
            t = (day - 1) * t_instances + period  # Calculate time instance number for this day and period
            time_instances.append(t)
    
    return time_instances


curlyT_actual = reservation_date_to_time_instances(start_reservation_date_str, end_reservation_date_str)
...
#Finding Tau - the last time instance in the reservation interval
def get_last_time_instance_of_reservation(start_reservation_date_str, end_reservation_date_str):
    time_instances = reservation_date_to_time_instances(start_reservation_date_str, end_reservation_date_str)
    curlyT = normalize_list(time_instances)
    return curlyT[-1] if curlyT else None

tau = get_last_time_instance_of_reservation(start_reservation_date_str, end_reservation_date_str)

# Finding curlyL
def service_date_to_days(start_date_str, end_date_str):
    
    # Convert a service date range to the corresponding days in the year. This returns a list of days.
...
#Since GetProbability functions reads through the full name.
def getRoom(C):
    switcher = {
        "CR": "Comfort Room",
...
def get_season(arrival_date):
    #To check in what season is the arrival_date
...
def get_suite(room_class):
    #If Not Suite return False
    if(room_class == "DCS" or room_class == "SIS" or room_class == "SS"  or room_class == "WS" or room_class == "LS"):
        return True
    else:
        return False
...
def get_month_from_time_index_t(t, t_instances):
    ...
    time_instance = curlyT_actual[t-1]
...
def getProb_requests(k, t):
    ...
    # Divide the probability by the number of time instances in the month
    total_time_instances = days_in_month * t_instances
    if total_time_instances != 0:  # Avoid division by zero
        return probability / total_time_instances
...
def getProb_cancellations(k, t):
    ...
    # Getting room class from reservation list
    room_type = getRoom(k.rrt)
    ...
    # Divide the probability by the total number of time instances in the month
    total_time_instances = days_in_month * t_instances
    if total_time_instances != 0:  # Avoid division by zero
        return probability / total_time_instances
...
for c in curlyC:  # Loop over room classes
    for i in curlyL:  # Loop over service days
        for t in range(2, tau + 2):  # Start from the second time instance
            m.addConstr(
